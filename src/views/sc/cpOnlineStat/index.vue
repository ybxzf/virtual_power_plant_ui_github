<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="cpOnlineStatList" @selection-change="handleSelectionChange">
...
<script>
import { listCpOnlineStat, getCpOnlineStat, delCpOnlineStat, addCpOnlineStat, updateCpOnlineStat } from "@/api/sc/cpOnlineStat";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        startTime: [
          { required: true, message: "开始统计时间不能为空", trigger: "blur" }
        ],
        endTime: [
          { required: true, message: "结束统计时间不能为空", trigger: "blur" }
        ],
        writeDate: [
          { required: true, message: "写入日期不能为空", trigger: "blur" }
        ],
        totalTime: [
          { required: true, message: "累计统计时间不能为空", trigger: "blur" }
        ],
        onlineTime: [
          { required: true, message: "在线时间不能为空", trigger: "blur" }
        ],
        offlineTime: [
          { required: true, message: "掉线时间不能为空", trigger: "blur" }
        ],
        offlineTimes: [
          { required: true, message: "掉线次数不能为空", trigger: "blur" }
        ],
        onlineRate: [
          { required: true, message: "在线率不能为空", trigger: "blur" }
        ],
        commNumber: [
          { required: true, message: "通信次数不能为空", trigger: "blur" }
        ],
        commRate: [
          { required: true, message: "通信成功率不能为空", trigger: "blur" }
        ],
        dataFlag: [
          { required: true, message: "统计时是否有当天的数据不能为空", trigger: "blur" }
        ],
        loginTimes: [
          { required: true, message: "日登录次数不能为空", trigger: "blur" }
        ],
        totalFlux: [
          { required: true, message: "累计流量不能为空", trigger: "blur" }
        ],
        upflux: [
          { required: true, message: "累计上行流量不能为空", trigger: "blur" }
        ],
        dnflux: [
          { required: true, message: "累计下行流量不能为空", trigger: "blur" }
        ],
        hbtFlux: [
          { required: true, message: "心跳登陆流量不能为空", trigger: "blur" }
        ],
        mpRunCount: [
          { required: true, message: "接入运行测量点数不能为空", trigger: "blur" }
        ],
        mpDataCount: [
          { required: true, message: "有数据测量点数不能为空", trigger: "blur" }
        ]
      }
    };
  },
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除终端日在线情况编号为"' + ids + '"的数据项？').then(function() {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('sc/cpOnlineStat/export', {
        ...this.queryParams
